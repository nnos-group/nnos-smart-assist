...
export const RecommendationReasonCard: React.FC<RecommendationReasonCardProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* RAZÃO DA RECOMENDAÇÃO & RESPOSTAS RELACIONADAS */}
      <div className="bg-sky-50/70 rounded-xl p-3 border border-sky-100 space-y-1.5 text-xs">
        <div className="flex items-start gap-1.5 text-slate-700 font-medium">
          ...
          <p className="leading-tight">{reason}</p>
        </div>

        {/* Respostas da entrevista relacionadas */}
        {relatedAnswers.length > 0 && (
          <div className="flex flex-wrap gap-1 pt-1">
            {relatedAnswers.map((ans) => (
              <span
                key={ans}
                className="px-2 py-0.5 rounded bg-white text-[10px] font-semibold text-sky-900 border border-sky-200/80"
              >
                ✓ {ans}
              </span>
            ))}
      ...
      {/* PROBLEMA QUE RESOLVE & BENEFÍCIO ENTREGUE */}
